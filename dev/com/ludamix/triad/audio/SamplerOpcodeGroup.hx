...
class SamplerOpcodeGroup
{
	...
	{
		// the two key differences between this and SFZ:
		
		// SF2 has additive generators - the instrument lies on top of the preset parameters.
		// SF2 has global zones per instrument in addition to the preset/instrument distinction.
		...
		for (region in regions)
		{
			...
			for (directives in [preset_opcodes, group_opcodes, region])
			{
			...
			//if (StringTools.startsWith(sampler_patch.name,"Orch"))
			//	trace([sampler_patch.name, sampler_patch.volume, vol_db, group_opcodes.get("volume")]);
