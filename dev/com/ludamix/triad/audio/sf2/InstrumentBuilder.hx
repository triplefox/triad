...
{
    private var _lastInstrument:Instrument;
    ...
    public override function read(d:ByteArray) : Instrument
    {
        ...
        i.startInstrumentZoneIndex = d.readShort();
        if(_lastInstrument != null)
        {
            _lastInstrument.endInstrumentZoneIndex = (i.startInstrumentZoneIndex - 1);
        }
        data.push(i);
        _lastInstrument = i;
        return i;
    ...
    public function loadZones(zones:Array<Zone>)
    {
        ...
        for(instrument in 0 ... data.length - 1)
        {
            ...
            for (j in 0 ... (i.endInstrumentZoneIndex - i.startInstrumentZoneIndex + 1))
            {
                i.zones[j] = zones[i.startInstrumentZoneIndex + j];
            }
        }
